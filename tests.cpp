...
bool testBarDefaultConstructor() {
    ...
    cout << "testBarDefaultConstructor: all passed!" << endl << endl;
    return true;
}

// tests param bar constructor
bool testBarParamConstructor() {
    ...
    cout << "testBarParamConstructor: all passed!" << endl << endl;
    return true;
}

// tests all bar operators
void testBarOperator1(BarChart &bc) {
    ...
    if (bc[2] >= bc[1]) {
        cout << "Greater than or equal to passed!" << endl;
        count++;
    }
    if (bc[2] > bc[1]) {
...
// tests all bar operators
void testBarOperator2(BarChart &bc) {
...
void testBarChartConstuctorMatch1(BarChart &bc, Bar &b) {
...
// Tests equality operator and copy contructor between barcharts
void testBarChartConstuctorMatch2(BarChart &bc, BarChart &bc2) {
    cout << "TESTING BARCHART TO BARCHART EQUALITY (5 tests): " << endl;
    for (int i = 0; i < bc.getSize(); i++) {
        if (bc[i] == bc2[i]) {
            cout << "Match BarChart Test " << i << " passed!" << endl;
        } else {
            cout << "Match BarChart Test " << i << " failed!" << endl;
        }
    }
    cout << endl;
}
...
// tests default constructor, param constructor, copy constructor and copy operator
void testBC(BarChart &bc) {
    Bar b("a", 1, "cat");
    // tests BarChart to Bar
    testBarChartConstuctorMatch1(bc, b);
    
    // tests BC to BC Test 1
    BarChart bc2(bc);
    testBarChartConstuctorMatch2(bc, bc2);
    
    // tests BC to BC Test 2
    BarChart bc3 = bc;
    testBarChartConstuctorMatch2(bc, bc3);
...
// tests addFrame and Animate twice
